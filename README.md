...
```bash
./.venv/bin/python main.py [데이터 파일 경로]   # 경로 생략 시 기본값 data.json
```

실행하면 시료 관리 / 시료 주문 / 주문 승인·거절 / 모니터링 / 생산 라인 조회 /
출고 처리 / 종료 메뉴가 뜹니다. 종료를 선택하면 현재 상태가 지정한 파일에
저장되고, 같은 파일로 다시 실행하면 이전 상태가 그대로 복원됩니다.

## 더미(시드) 데이터 생성

시연/테스트용 데이터를 직접 입력하지 않고 만들고 싶다면 아래 명령을 별도로
실행합니다(자동 적재되지 않으며, 명시적으로 실행해야만 생성됩니다).

```bash
./.venv/bin/python -m sample_order.seed [경로] --samples 5 --orders 20 --seed 42
```

생성된 파일을 그대로 `main.py [경로]`로 열어볼 수 있습니다.
