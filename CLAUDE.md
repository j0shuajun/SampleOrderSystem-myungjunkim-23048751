...
승인은 총 두 번(plan 승인, 완료 승인) 있고, 그 사이에 커밋이 네 번
(plan/test/구현/result) 있다. **구현(GREEN) 커밋은 승인 없이 바로 한다** — 승인은
REVIEW(두 검증)까지 끝난 뒤 result 문서를 커밋하기 직전, 딱 한 번만 더 받는다.
push는 그 완료 승인 직후 Phase당 한 번이다.
...
[구현] 테스트를 통과시키는 최소 구현   ← GREEN
   │
   ▼
[커밋 #3] feat: / fix: / refactor:   (승인 없이 바로 커밋)
   │
   ▼
[검증] test-verifier ─┬─ 병렬 실행   ← REVIEW
       compliance-verifier ─┘
   │
   ├─ 하나라도 FAIL → 원인 파악 → 재작업 → 다시 커밋 → 다시 검증 (반복)
   │
   ▼ 둘 다 PASS
[문서] result 문서 작성 (docs/tasks/..._result.md)  — 아직 커밋 안 함
   │
   ▼
[승인 #2] 사람에게 요약 보고(구현내용/검증결과/커밋목록) → 승인   ★ 멈춤 지점 2
   │                                                    (result 커밋 전!)
   ▼ 승인
[커밋 #4] result 문서 docs: 커밋
   │
   ▼
[push] 원격 저장소에 push
   │
...
4. **사람 승인 없이 곧바로** 구현 커밋을 `feat:`, 버그 수정이면 `fix:`, 구조 개선이면
   `refactor:`로 남긴다. (승인은 REVIEW 이후, result 문서 커밋 전에 받는다.)
...
1. `test-verifier`와 `compliance-verifier`를 **병렬로** 실행한다.
   ...
   - 하나라도 FAIL이면 원인을 고치고 다시 커밋한 뒤 재검증한다.
2. 구현이 이번 Phase 범위를 넘지 않았는지, 불필요한 추상화·임의 기능·UI 과잉 구현이
   없는지 확인한다.
3. 두 검증이 모두 PASS이면 result 문서를 작성한다(아직 커밋하지 않는다).
4. **result 문서를 사람에게 보고하고 승인을 받는다.** Phase마다 변경 요약(무엇을
   구현했는지, 두 검증 결과, 커밋 목록)을 사람에게 보고하고, 명시적으로 승인받은
   뒤에만 result 문서를 `docs:` 커밋하고 push한다. 두 검증이 PASS여도 이 승인
   단계 자체를 생략하지 않는다.
