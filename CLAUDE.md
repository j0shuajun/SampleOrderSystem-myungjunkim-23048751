...
3. `ai-action` 서브에이전트(또는 동일한 원칙을 따르는 직접 작업)로 실패해야 하는
   테스트를 작성하고, 실제로 실패하는지 확인한다.
4. 테스트와 문서 변경을 `test:` 또는 `docs:` 커밋으로 남긴다.
...
2. 두 검증을 모두 통과한 뒤에만 사람의 코드 리뷰로 넘긴다.
3. 구현이 이번 Phase 범위를 넘지 않았는지, 불필요한 추상화·임의 기능·UI 과잉 구현이
   없는지 확인한다.
...
- 재고 부족 시 실제 생산량은 `ceil(부족분 / 수율)`로 계산한다. 부족분 외에 별도의
  안전재고(버퍼)는 추가하지 않는다 — 이 수식이 이미 수율 손실을 보정한 최종 생산량이다.
