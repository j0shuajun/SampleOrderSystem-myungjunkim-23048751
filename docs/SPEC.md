...
- 생산 작업에는 주문 ID, 시료 ID, 부족분, 계획 생산량, 작업 시작 시각이 포함된다.
  예상(총) 생산 시간은 별도 필드로 저장하지 않고, `평균생산시간 × 계획생산량`으로
  필요할 때마다 계산한다(13장 예시 JSON 스키마와 일치).
...
생산 작업(Production Job) 상태:

| 상태 | 의미 |
|---|---|
| `QUEUED` | 큐에 등록됨, 아직 시작 전 (`started_at`은 `null`) |
| `IN_PROGRESS` | 생산 라인이 시작함 (`started_at`에 시작 시각 기록) |
| `DONE` | 완료 처리됨 |

생산 작업 ID는 `JOB-` + 4자리 일련번호 형식(`JOB-0001`)을 쓴다(12장 ID 포맷 참고).
...
판정 순서(위에서부터 먼저 만족하는 조건을 적용한다 — 세 조건은 서로 배타적이다):

| 순서 | 상태 | 기준 |
|---|---|---|
| 1 | 고갈 | 현재 재고가 0 (미출고 주문 수량과 무관하게 최우선 적용) |
| 2 | 여유 | (재고 > 0이고) 현재 재고가 미출고 주문 수량 이상 |
| 3 | 부족 | (재고 > 0이고) 현재 재고가 미출고 주문 수량보다 적음 |

예: 재고 0, 미출고 주문 0(대기 수요 자체가 없는 시료)이어도 "고갈"로 표시한다 —
재고가 물리적으로 없다는 사실이 수요 유무보다 우선한다.
...
- 주문
- 생산 작업
- 재고 — 별도 구조가 아니라 각 시료 레코드의 `stock` 필드로 저장된다("재고"는
  시료와 분리된 4번째 저장 대상이 아니라, 시료 데이터의 일부다).

기본 저장 방식은 JSON 파일을 우선 고려한다. 외부 DB는 필수로 두지 않는다.
저장 파일 경로는 기본값(`data.json`)을 두되, 실행 시 지정 가능하게 한다
(DataPersistence PoC와 동일한 방식).

`ProductionJob.started_at`처럼 JSON이 기본 지원하지 않는 값(datetime)은 ISO 8601
문자열로 저장하고, 값이 없으면(`None`, 아직 시작 전) JSON의 `null`로 저장한다.
...
| 생산 작업 ID | `JOB-` + 4자리 일련번호 | `JOB-0001` |
...
`started_at`은 작업이 `IN_PROGRESS`/`DONE`이 되면 ISO 8601 문자열(예:
`"2026-07-15T09:00:00"`)로 저장된다.
