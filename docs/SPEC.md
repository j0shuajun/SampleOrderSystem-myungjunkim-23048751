...
```text
committed = 이 시료를 대상으로 하는, 지금 심사 중인 주문을 제외한 다른 모든
            미출고 주문(RESERVED, PRODUCING, CONFIRMED 상태)의 수량 합계
available = max(0, sample.stock - committed)
```

`committed`는 8장(모니터링)의 "미출고 주문 합계"와 동일한 정의를 그대로 재사용한다.

우선순위 원칙: **먼저 승인(또는 접수)되어 미출고 상태가 된 주문이 재고에 대한 우선권을
...
"미출고 100"으로 committed에 잡힌다. 이어서 주문 B(20)를 심사하면
`available = max(0, 70 - 100) = 0`이므로 B는 재고가 있어 보여도(raw stock=70) 즉시
`CONFIRMED`되지 않고, 부족분 20에 대해 별도로 `PRODUCING`에 들어간다.
...
- 시간 소스(현재 시각)는 테스트에서 mocking 가능하도록 별도 함수/의존성으로 분리한다.
