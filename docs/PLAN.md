...
- `main.py`가 시작 시 `Repository.load_into(...)`로 기존 데이터를 불러오고,
  "종료" 메뉴 선택 시 `Repository.save(...)`로 저장한 뒤 종료한다(SPEC.md 9장,
  10장의 "0 종료: 저장 후 종료"와 PRD 7장의 "재실행 후 유지" 요구를 실제로 연결).
- 저장 파일 경로는 DataPersistence PoC와 동일한 방식을 따른다: `main.py`를
  실행할 때 위치 인자로 경로를 받고(`python3 main.py [path]`), 생략하면 기본값
  `data.json`을 쓴다.
...
- 프로그램을 종료했다가 같은 경로로 재실행하면 이전 데이터가 그대로 보인다.
